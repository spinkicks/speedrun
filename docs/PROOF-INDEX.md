...
| **§7e leakage check** (scan training data for leaked test items → clean) | ✅ | `RESULTS.md §7e` + `services/speedrun-ai/eval/leakage-check.json`: **0 leaks — CLEAN** (181 training items vs 50 gold; 0 verbatim + 0 cosine on test-item identity; 1 honest strict-diagnostic flag on the canonical 2×2 determinant step, disclosed) | `cd services/speedrun-ai && uv run python -m eval.leakage_check` ; `uv run --extra dev python -m pytest tests/test_leakage_check.py` |
