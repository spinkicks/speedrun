...
- Interactive-MCQ auto-grading (P2-D deferred): `Speedrun::Problem` is a show-answer/self-rate card — revlog stores only `button_chosen`, never the chosen option, so Performance "correct" is self-assessed, not key-checked. Real fix = clickable choices in card JS → compare to `CorrectAnswer` → persist correctness where the engine reads it, on desktop + Android (its own both-platforms feature). Interim shipped instead: self-grade-against-key afmt copy + "self-reported until interactive grading" honesty caveat. [P2-D, 2026-07-02]
