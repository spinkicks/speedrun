...
**Status:** DRAFT for David's review (2026-07-02). Slice B of the frontend/UX revamp (see `FUTURE-PLANS.md` → "DISTINCT-IDENTITY MANDATE"). Mockup: `docs/design/mockups/speedrun-identity.html`. Supersedes the type/accent choices in `speedrun-home-spec.md` §"Design language" (that spec's layout/signature-element still stand; only the typeface + accent color change).
...
- **Display / wordmark / headings:** a strong, professional sans at heavy weight. **Recommend Inter** (Bold/Black) — trusted, clean, geometric-humanist; reads "serious product," not "terminal." (Alternate if you want more character: Manrope ExtraBold or Archivo Black — pick from the mockup.)
...
- Must be **offline-safe** in the Anki webview — self-host/bundle the woff2 (Inter is OFL-licensed; ships fine) with a strong system fallback stack (`"Inter", "Segoe UI", system-ui, sans-serif`).
...
**3. Wordmark.** `SPEEDRUN` set in the heavy display sans, tight tracking, solid **white** (drop the two-tone amber "RUN"). Optional subtle differentiator: a thin white rule/tick as a "speedrun timer" mark rather than a color split. Lockup + subtitle `GRE MATHEMATICS · SUBJECT TEST` in `--muted`, sans (not mono).
...
3. **App icon / launcher icon** — a Speedrun mark (monogram `S` timer/chevron or the wordmark lockup). See mockup for the mark direction; final asset TBD with David.
