...
4. **Auto-open placement:** the current auto-open fires right after `gui_hooks.profile_did_open()` and BEFORE `maybe_auto_sync_on_open_close()` (`qt/aqt/main.py` ~L557) — this can stack the sync-progress dialog under Home. Move it into the post-sync `_onsuccess` callback, and skip auto-open when `self.safeMode` is set.
