...
## Current data state for THIS recording (read before shooting)
This profile has **problem** data but not **declarative** data, so **Performance (41%) and Readiness (521, band 478–563) are LIVE**, while **Memory abstains ("—") and The Map is grey**. That is correct and on-thesis, not a bug: Memory is recall of the *declarative* cards (definitions/theorems) and is computed **only** from those — it deliberately ignores problem cards. The ~79 reviews here were **problem** attempts (Start Run + mini-mocks), which feed Performance/Readiness but not Memory. Memory unlocks per topic only after **≥20 graded reviews on ≥2 declarative cards**, so every topic shows "—", and since The Map is colored by Memory recall, its nodes are grey (the graph structure + blast-radius tap still work). **Lean into this on camera as the honesty rule** ("it won't show a number it can't back up"). Do NOT try to fake/force Memory for the shot.
...
**2 — Three scores + Readiness gauge.** "Three scores, each earned from real data — and each abstains when it isn't there yet. Right now Performance *points at 41%* and Readiness *points at the gauge — 521 on the real 200-to-990 exam scale, with its 478-to-563 band for the uncertainty* are live, because I've done timed problems. Memory *points at the '—'* honestly shows a dash: it measures something different — recall of the definition cards — and I haven't studied those yet. It maps a score onto the actual exam scale, with a band, and it flat-out refuses to invent a number it can't back up."

**3 — THE MAP.** "This is THE MAP *clicks "The Map" in the left sidebar* — the prerequisite graph of the whole subject. Every node is colored by your real recall, and right now they're grey — that's the same honesty rule: I haven't studied the definitions, so it says '—' instead of guessing. But the structure and the interaction work regardless *taps Calculus* — tap a topic and its blast radius lights up *gestures at the highlighted downstream nodes*, every topic it's holding back. Fix the root, and you lift everything above it. Once you study a topic's definitions, its node takes on a real mastery color."
...
**8 — Memory→Performance gap.** "And the gap we care about most *scrolls to the Memory→Performance gap panel* — recalled memory versus timed accuracy on the same topic. Right now it honestly says there's nothing to show yet *points at the 'No gap to show yet' message* — it needs both signals on one topic: memory from studying the definitions, performance from timed problems. That's the point — when both exist and the line drops, you remember it but can't use it under time. No data, no line: we don't draw a gap we haven't measured."
