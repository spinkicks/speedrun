...
- **One engine, two apps:** the same forked `rslib` compiles to desktop (PyO3) and Android (JNI AAR via cargo-ndk); proven by an instrumentation test asserting identical backend version and by the same UI rendering on both. **Phase 6:** rsdroid re-pinned to the `c54afe2b1`-era engine, **AAR rebuilt**, and the **Android APK compiled with the Speedrun UI bundled in the AAR (verified)**. ✅ real.
