...
- **Watch Claude's output:** hook writes digests to `.claude/watch.log` (project-level `.claude/settings.json` hook — survives Claude restarts). Reconnect: background-run `Get-Content .claude/watch.log -Wait -Tail 0` with output-notify on `\]\[(Stop|SubagentStop)\]`. For live detail read the newest transcript: `~/.claude/projects/C--Users-davir-Ultra-Alpha-Speedrun/<newest>.jsonl` (currently `01774af5-…`).
