...
- **Open:** (a) David's **Android emulator smoke** (post-merge confirmation, steps in RUN-MVP §B — AAR already built) + recordings; (b) non-critical audit backlog in `FUTURE-PLANS.md` `[audit]`; (c) **Thursday:** Claude grounds `docs/plans/2026-07-03-friday-brief.md` into a full Friday plan (3 scores + AI service + sync demo) for Cursor review.
- **Optional infra:** `.claude/cursor-review-notify.ps1` (UserPromptSubmit nudge hook) written but NOT wired into `settings.json` — David relays manually for now (working well).
