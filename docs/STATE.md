...
- **Friday plan READY for Cursor review (do NOT execute yet):** `docs/plans/2026-07-03-friday-ai-scores-sync.md` — Claude grounded it (6-agent sweep, every file:line verified); design decisions D1–D4 (in-engine deterministic scoring, recompute-on-read, extend proto with `scale` enum, mini-mock via filtered deck). **Cursor's full gate review = Thursday.** Highest-weight day (AI 15% + scores 20% + Rust/study remaining halves + sync 10%).
- **Open:** (a) David's Wednesday recordings + submission; (b) audit backlog in `FUTURE-PLANS.md` `[audit]`; (c) Thursday: Cursor reviews the Friday plan → approve → execute.
