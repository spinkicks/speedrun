...
**NOW EXECUTING — Mobile-first UX + START RUN fixes + reviewer restyle** on new branch **`feat/speedrun-mobile-first`** (off `main`). David's on-device testing found: desktop START RUN dead-ends into stock Anki "Congratulations"; Android START RUN is a no-op; mobile layout wraps one-word-per-line. Cursor-authored brief + Claude-grounded executable plan, **APPROVED by Cursor** (`docs/plans/2026-07-01-mobile-first-and-startrun-plan.md`; brief: `…-startrun.md`). Order **M0 (mobile-first responsive) → M1 (Android dark shell) → S1 (desktop START RUN + honest fallbacks + Custom Study) → S2 (Android START RUN via `bridgeCommand`) → R1 (FULL reviewer chrome restyle, both platforms, presentation-only)**. Screenshot gate (mobile ~360px + desktop / emulator) every phase; Cursor FF-merges each. **R1 is the risk to watch — descope first if it threatens Friday's scoring work.**
- **Open:** (a) recordings (desktop demo-ready now; wait for mobile-first + START RUN fix for the phone recording); (b) non-critical audit backlog in `FUTURE-PLANS.md` `[audit]`; (c) **Thursday:** Claude grounds `docs/plans/2026-07-03-friday-brief.md` into a full Friday plan (3 scores + AI service + sync demo) for Cursor review.
