...
- **Current fork tips (2026-07-03 PM):** the SHAs above are the **feature-complete** commits; each fork `main` now also carries one **docs-only Speedrun-fork README banner** commit on top → current tips are anki **`ce28bb0fd`**, anki-android **`f02bde8366`**, Anki-Android-Backend **`70b8eaf`**. Umbrella `main` = latest docs-only planning-doc refresh (no code delta vs the feature SHAs).
